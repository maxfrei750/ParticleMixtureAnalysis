...
def test_model_on_dataset(
    ...
    :param model_id: ID of the model to use. If None, then the latest model will be used.
    """

    data_root = Path(data_root)
    log_root = Path(log_root) / config_name
    output_root = Path(output_root) / config_name
    class_selector = get_class_selector(config_name)
...
def get_checkpoint_path(log_root: AnyPath, model_id: Optional[str] = None) -> AnyPath:
...
def get_class_selector(config_name: str) -> Optional[List[str]]:
    ...
    :param config_name: Name of the config used for the training of the model.
    :return: List that can be used as class selector for a data set or None.
    """
    if "light" in config_name.lower():
        class_selector = ["light"]
    elif "dark" in config_name.lower():
        class_selector = ["dark"]
    else:
        class_selector = None
